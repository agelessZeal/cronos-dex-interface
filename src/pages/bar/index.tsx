...
import { GATE, SUSHI, XSUSHI } from '../../config/tokens'
...
function Stake() {
  ...
  const [ratio, totalSupply] = useGateBar()

  const xSushiPerSushi = ratio //parseFloat(data?.bar?.ratio)
  ...
  useEffect(() => {
    ...
    if (ratio && totalSupply) {
      const gateRatio = ratio as number

      const gatePrice = 0.1
      const volumeUSD = 100

      const gateTotalSupply = parseFloat(totalSupply.toFixed())

      const apr = (((volumeUSD * 0.05) / gateTotalSupply) * 365) / (gateRatio * gatePrice)
  ...
  return (
    <Container id="bar-page" className="py-4 md:py-8 lg:py-12" maxWidth="full">
      <Head>
        <title key="title">Stake | Gate</title>
        <meta
          key="description"
          name="description"
          content="Stake GATE in return for xGATE, an interest bearing and fungible ERC20 token designed to share revenue generated by all GATE products."
        />
        <meta key="twitter:url" name="twitter:url" content="https://app.sushi.com/stake" />
        <meta key="twitter:title" name="twitter:title" content="STAKE GATE" />
        <meta
        ...
        <meta key="og:title" property="og:title" content="STAKE GATE" />
          ...
          <div className="flex flex-col w-full max-w-xl mt-auto mb-2">
            <div className="flex max-w-lg">
              <div className="self-end mb-3 text-lg font-bold md:text-2xl text-high-emphesis md:mb-7">
                {i18n._(t`Maximize yield by staking GATE for xGATE`)}
              </div>
            ...
            {/* <div className="max-w-lg pr-3 mb-2 text-sm leading-5 text-gray-500 md:text-base md:mb-4 md:pr-0">
              {i18n._(t`For every swap on the exchange on every chain, 0.05% of the swap fees are distributed as GATE
                                proportional to your share of the GateBar. When your GATE is staked into the GateBar, you receive
                                xGATE in return for voting rights and a fully composable token that can interact with other protocols.
                                Your xGATE is continuously compounding, when you unstake you will receive all the originally deposited
                                GATE and any additional from fees.`)}
            </div> */}
            {/* <div className="flex">
                            ...
                            <div>
                                <StyledLink className="text-sm text-lg whitespace-nowrap md:text-lg md:leading-5">
                                    Tips for using xGATE
                                </StyledLink>
                            </div>
                        </div> */}
          </div>
          {/* <div className="hidden px-8 ml-6 md:block w-72">
            <Image src="/xsushi-sign.png" alt="xGATE sign" width="100%" height="100%" layout="responsive" />
          </div> */}
        </div>
              ...
              <div className="w-full max-w-xl px-3 pt-2 pb-6 rounded bg-dark-900 md:pb-9 md:pt-4 md:px-8">
                ...
                <div className="flex items-center justify-between w-full mt-6">
                  <p className="font-bold text-large md:text-2xl text-high-emphesis">
                    {activeTab === 0 ? i18n._(t`Stake GATE`) : i18n._(t`Unstake`)}
                  </p>
                ...
                <div className="relative w-full h-0 pointer-events-none bottom-14">
                  ...
                  >
                    <div className="flex space-x-2 ">
                      ...
                      >
                        {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}GATE`}
                      </p>
                    </div>
                ...
                <div className="flex flex-col flex-grow md:mb-14">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex flex-col justify-center">
                      ...
                      <p className="text-sm md:text-base text-primary">xGATE</p>
                    </div>
                  </div>
                </div>
